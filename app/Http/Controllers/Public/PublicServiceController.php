...
{
    // ✅ Clinic rules
    private const CLINIC_OPEN  = '09:30';  // ✅ start at 9:30 AM
    private const CLINIC_CLOSE = '17:00';  // ✅ close at 5:00 PM
    ...
    private function computeHourlySlots(string $date, ?int $doctorId): array
    {
        ...
        // Today lead time (aligned to clinic start grid at 09:30)
        ...
        if ($openStart->isSameDay($now)) {
            ...
            if ($minCandidate->gt($openStart)) {
                $diff = $openStart->diffInMinutes($minCandidate, false);
                $n = (int) ceil($diff / self::SLOT_MINUTES);
                $minStart = $openStart->copy()->addMinutes($n * self::SLOT_MINUTES);
            }
        }
