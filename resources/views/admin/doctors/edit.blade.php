...
<style>
    ...
    html[data-theme="dark"] .meta{ background: rgba(2,6,23,.25); }
    ...
    .form-card{ padding: 16px; margin-top: 12px; }
...
<div class="cardx meta-card">
    <div class="meta-grid">
        <div class="meta">
            <div class="k">Doctor ID</div>
            <div class="v">#{{ $doctor->id }}</div>
        </div>
        <div class="meta">
            <div class="k">Created</div>
            <div class="v">{{ $doctor->created_at?->format('M d, Y') ?? '—' }}</div>
        </div>
        <div class="meta">
            <div class="k">Status</div>
            <div class="v">{{ $doctor->is_active ? 'Active' : 'Inactive' }}</div>
        </div>
    </div>
</div>
...
<div class="cardx form-card">
    <div class="section">
        <div>
            ...
            <div class="hint">Updates will reflect immediately in Staff side.</div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.doctors.update', $doctor) }}">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <input class="form-control kt-control" name="name" value="{{ old('name', $doctor->name) }}" required>
            </div>

            <div class="col-md-6">
                <label class="form-label">Specialty</label>
                <input class="form-control kt-control" name="specialty" value="{{ old('specialty', $doctor->specialty) }}" placeholder="e.g. Dentist / Orthodontist">
            </div>

            <div class="col-md-6">
                <label class="form-label">Email (optional)</label>
                <input class="form-control kt-control" type="email" name="email" value="{{ old('email', $doctor->email) }}" placeholder="doctor@email.com">
            </div>
                ...
                <div class="toggle-wrap">
                    ...
                    <div class="form-check form-switch" style="margin:0;">
                        <input class="form-check-input" type="checkbox" name="is_active" value="1" id="is_active"
                               @checked(old('is_active', $doctor->is_active) ? true : false)>
